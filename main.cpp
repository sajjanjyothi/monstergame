...
int deployMonsters(int count,vector <City> &mapVector, vector <Monster> &monsterVector)
{
    ...
    while(count > 0)
    {
        ...
        else //both monsters met each other in initialisation itself , destroy city and monster
        {
           ...
           auto monstIt1 = find_if(monsterVector.begin(), monsterVector.end(),[&](Monster a){ return a.getNumber() == city.getMonster1();});
           if(monstIt1 != monsterVector.end())
           {
                 monsterVector.erase(monstIt1);
           }  
           auto monstIt2 = find_if(monsterVector.begin(), monsterVector.end(),[&](Monster a){ return a.getNumber() == city.getMonster2();});
            ...
            switch(num)
            {
                ...
                {  
                    if( it != mapVector.end())
                    {
                        ...
                        {      
                            auto monstIt1 = find_if(monsterVector.begin(), monsterVector.end(), [&](Monster a) { return a.getNumber() == it->getMonster1(); });
                            if (monstIt1 != monsterVector.end())
                            {
                                monsterVector.erase(monstIt1);
                            }
                            auto monstIt2 = find_if(monsterVector.begin(), monsterVector.end(), [&](Monster a) { return a.getNumber() == it->getMonster2(); });
                            if (monstIt2 != monsterVector.end())
                            {
                                monsterVector.erase(monstIt2);
                            }
